...
export default function Requests({ _id, name, role }) {
  ...
  useEffect(() => {
    ...
    const bookLogger = bookLog.filter(book => book.borrower === _id);
    setLog(bookLogger);
    return () => {
  ...
  const filteredRequests = () =>
    requests.filter(request => {
      if (search !== "") {
        return (
          request.title.toLowerCase().indexOf(search.toLowerCase()) !== -1 ||
          request.author.toLowerCase().indexOf(search.toLowerCase()) !== -1 ||
          request.user.email.toLowerCase().indexOf(search.toLowerCase()) !== -1
        );
      } else {
        return book;
      }
                ...
                <Table className={classes.table} aria-label="customized table">
                  <TableHead>
                    <TableRow>
                      ...
                      <StyledTableCell align="right">Requestor</StyledTableCell>
